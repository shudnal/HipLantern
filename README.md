...
`Switch sound volume` controls the one-shot light/heat toggle sounds and defaults to `0.5`. Toggle sounds are positional world sounds and are emitted once per actual lantern toggle. Switch effects are local audio instances synchronized by HipLantern, preventing the vanilla effect prefabs from being network-replicated a second time.
